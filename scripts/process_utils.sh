...
# Process utilities for etl interactive script
...
process_single_episode() {
    ...
    if [ -f "$audio_file" ]; then
        ...
        # Use speaker-level segmentation (only mode)
        echo "🎯 使用說話人級別分段模式"
        
        local segmentation_args=""
        if [ -n "${MIN_SPEAKER_DURATION:-}" ]; then
            segmentation_args="$segmentation_args --min_speaker_duration $MIN_SPEAKER_DURATION"
        fi
...
process_and_split_menu() {
    ...
    echo ""
    echo -n "請輸入集數: "
    read -r episodes_input
    
    # Debug: 顯示讀取到的輸入
    echo "DEBUG: 讀取到的集數 = '$episodes_input'"
    
    # 檢查空輸入
    if [ -z "$episodes_input" ]; then
        echo "❌ 錯誤: 必須輸入集數"
        echo "請重新執行並輸入有效的集數（例如: 1, 1-5, 1 3 5, 或 all）"
        return 1
    fi
